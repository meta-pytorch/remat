...
class MemoryReportTest(expecttest.TestCase):
    def test_addressed_intervals_creates_no_reference_cycle(self) -> None:
        # _addressed_intervals used to walk a view's non-contiguous outer dims with a
        # recursive nested closure. A function that calls itself is captured in its own
        # closure cell, so every report call reaching this path spun up a self-referential
        # walk <-> cell cycle -- gc-collectable, but pure churn on a hot path. The
        # iterative rewrite creates no nested function at all. A column slice gives a
        # single contiguous inner run (size 2) plus one non-contiguous outer dim (size 3),
        # i.e. exactly the fanned-out branch the old walk handled.
        view = torch.arange(12).reshape(3, 4)[:, :2]
        self.assertEqual((3, 2), tuple(view.shape))
        self.assertEqual((4, 1), view.stride())
        assert_no_cycles_from(self, lambda: _addressed_intervals(view), "_reporting.py")
