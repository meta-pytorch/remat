    ...
    def decorate(function: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def checkpointed_function(*args: Any, **kwargs: Any) -> Any:
            return _torch_checkpoint_with_forward_exception_cleanup(
                ...
                context_fn=lambda: _checkpoint_context_fn(region_name, detect_bare_ops),
...
def save_for_backward(
    ...
    in_save_op = _active_save_op.get() is not None
    for tensor_name, tensor in saved.items():
        ...
        if in_save_op and isinstance(tensor, torch.Tensor):
            # Tag the tensor itself: pack reads this name back off the exact tensor it
            # is handed (see :func:`_default_pack`), so a name cannot shift onto
            # another op's tensor. Gated on being inside a SAVE op so a plain
            # ``save_for_backward`` outside a remat region tags nothing.
            setattr(tensor, _SAVE_NAME_ATTR, tensor_name)
...
class _SaveOpForwardScratch:
    ...
    # is persisted eagerly (Direction 1 in :func:`_prepare_outputs`) at no memory cost.
...
def _run_save_op(
    ...
    * otherwise identity: the tensor stays a normal autograd saved tensor on
    ...
    or a placeholder when none was saved (see :func:`_recompute_output`).
    """
    ...
    if state.phase is _Phase.RECOMPUTE:
        ...
        return _recompute_output(record, region_state)
    ...
    def pack(tensor: torch.Tensor) -> object:
        ...
        # memory-report row -- the payload is opaque.
        hooks = _active_saved_tensors_hooks.get()
        if hooks is not None:
            pack_hook, unpack_hook = hooks
            return _SavedHookData(pack_hook(tensor), unpack_hook)
...
# Attribute under which remat.save_for_backward tags a tensor with its report name;
# pack reads it back off the exact tensor autograd hands it.
_SAVE_NAME_ATTR = "_torch_remat_save_name"
...
def _resolve_save_name(scratch: _SaveOpForwardScratch, tensor: torch.Tensor) -> str:
    ...
    The :func:`save_for_backward` name read off the tensor's ``_SAVE_NAME_ATTR`` tag
    if it has one, else a positional ``saved.<i>`` from the op's unnamed-save counter.
    """

    _assert_phase(_Phase.FORWARD)

    name = getattr(tensor, _SAVE_NAME_ATTR, None)
    if name is None:
...
def _default_pack(
    ...
    # sharing it can be eagerly persisted (Direction 1 in :func:`_prepare_outputs`).
...
def _prepare_outputs(
    ...
    Direction 1: an output that is *itself* saved for backward (its storage is in
...
class _PersistOutputThunk:
    ...
    strategy, a ``remat.op`` consumer via the handle, or the eager Direction-1 save.
