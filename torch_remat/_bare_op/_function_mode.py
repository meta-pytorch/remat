...
class _SaveFunctionMode(TorchFunctionMode):
    ...
    def __torch_function__(
        ...
        versions = [tensor._version for tensor, _handle in touched]
        out = func(*args, **kwargs)
        if any(
            tensor._version != version
            for (tensor, _handle), version in zip(touched, versions)
        ):
            raise RuntimeError(_inplace_message(func))

        base_storage = _storage_id(touched[0][0])
        if base_storage and _all_alias(out, base_storage):
            _defer_view(
                self._region_state, out, touched[0][1].persist_output, touched[0][0]
            )
            return out
