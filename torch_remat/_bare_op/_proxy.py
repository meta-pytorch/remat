...
def _dispatch_proxy_op(
    ...
    if any(inner._version != version for inner, version in zip(inners, versions)):
        raise RuntimeError(_inplace_message(getattr(func, "__name__", func)))

    if base_storage and _all_alias(out, base_storage):
        # Defer: the view rides the producer's save, retaining the tensor it was derived
        ...
        persist_output = _BaseRetainingPersist(saves[0], inners[0])
        return map_value(
