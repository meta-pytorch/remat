...
class _SaveTensor(torch.Tensor):
    ...
    def __torch_dispatch__(
        ...
        # Reject an op that would mutate a SAVE-output wrapper in place: it corrupts both
        # the persisted value and the copy autograd kept for the SAVE op's backward. An
        # op that only reads a wrapper (it sits in a read-only operand) is fine.
        if _mutates_selected(func, args, kwargs, _is_save_tensor):
...
def _is_save_tensor(value: object) -> bool:
    """Whether a ``__torch_dispatch__`` leaf is a SAVE-output wrapper."""

    return isinstance(value, _SaveTensor)
