...
class _SaveDispatchMode(TorchDispatchMode):
    ...
    argument; if found, an op that would *write* one in place is rejected, else the
    producer's persist-output fires before the op runs on the plain arguments.
    """
    ...
    def __torch_dispatch__(
        ...
        index = self._region_state.save_output_index
        handles = _touched_save_handles(index, args, kwargs)
        if handles:
            # Reject only when the op writes a SAVE output in place; an op that merely
            # reads one (it sits in a read-only operand) is fine -- persist and run it.
            if _mutates_selected(
                func, args, kwargs, lambda value: _is_save_output(index, value)
            ):
...
def _is_save_output(index: Any, value: object) -> bool:
    """Whether a leaf is a SAVE output registered in the region's index.

    The tensor check guards ``WeakTensorKeyDictionary.get``, which raises on a non-tensor key.
    """

    return isinstance(value, torch.Tensor) and index.get(value) is not None
